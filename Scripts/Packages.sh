...
# Keep in-tree dae package (DaeWRT) alongside cloned luci-app-dae
if [ -d "$GITHUB_WORKSPACE/package/dae" ]; then
	rm -rf ./dae
	cp -r "$GITHUB_WORKSPACE/package/dae" ./
	echo "package/dae synced from YanWRT tree"
fi

# EdgePi E87N fancontrol stack from Yanw3n/imagebuilder
if [ -d "$GITHUB_WORKSPACE/package/e87n" ]; then
	rm -rf ./e87n
	cp -r "$GITHUB_WORKSPACE/package/e87n" ./
	echo "package/e87n (fancontrol) synced from YanWRT tree"
fi
